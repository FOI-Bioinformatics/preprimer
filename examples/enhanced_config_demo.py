...
from preprimer.core.enhanced_config import (
    EnhancedConfig, AlignmentSettings, ValidationSettings, 
    OutputSettings, ParserSettings, ConfigManager, get_config, update_config_partial
)
...
def demo_basic_configuration():
    ...
    print(f"Primer length range: {config.validation.min_length}-{config.validation.max_length}")
...
def demo_runtime_reconfiguration():
    ...
    def log_changes(new_config):
        change_log.append(f"Config changed: debug_mode={new_config.debug_mode}, threads={new_config.alignment.threads}")
    ...
    manager.update_partial(
        debug_mode=False,
        **{
            "validation.min_length": 22,
            "output.force_overwrite": True
        }
    )
...
def demo_validation_features():
    ...
    try:
        ...
        EnhancedConfig(
            alignment=AlignmentSettings(aligner="invalid_tool")
        )
    ...
    try:
        ...
        EnhancedConfig(
            validation=ValidationSettings(min_length=25, max_length=25)
        )
        print("✗ Should have failed!")
    ...
    try:
        ...
        EnhancedConfig(
            output=OutputSettings(formats=["invalid_format"])
        )
        print("✗ Should have failed!")
...
def demo_legacy_compatibility():
    ...
    print(f"  Primer length: {legacy_config.min_primer_length}-{legacy_config.max_primer_length}")
