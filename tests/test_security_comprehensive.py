        ...
        try:
            ...
            for component in components:
                if component in ("..", "."):
                    ...
                    with pytest.raises(
                        SecurityError, match="Path traversal attempt detected"
                    ):
                        raise SecurityError(
                            f"Path traversal attempt detected: {component}"
                        )
...
class TestSecureFileOperationsEdgeCases:
    ...
    def test_safe_remove_tree_system_directory_protection(self):
        ...
        with patch.object(ops, "_is_system_directory") as mock_is_system:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                with pytest.raises(
                    SecurityError, match="Refusing to remove system directory"
                ):
...
class TestSecureSubprocessEdgeCases:
    ...
    def test_secure_subprocess_cwd_validation(self):
        ...
        with patch(
            "preprimer.core.security.PathValidator.sanitize_path"
        ) as mock_sanitize:
...
class TestSecureFileOperationsSystemDirectoryDetection:
    ...
    def test_is_system_directory_unix_paths(self):
        ...
        system_dirs = ["/bin/bash", "/etc/passwd", "/root/secret", "/usr/bin/ls"]
...
class TestPathValidatorComplexScenarios:
    ...
    def test_sanitize_path_system_directory_restrictions(self):
        ...
        for sys_path in system_paths:
            with pytest.raises(
                SecurityError, match="Access to system directory denied"
            ):
    ...
    def test_path_components_validation_comprehensive(self):
        ...
        try:
            validator.validate_path_components(
                Path("/home") / "user" / ".." / "root"
            )  # Path traversal
