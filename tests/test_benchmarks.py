...
from preprimer.core.enhanced_config import EnhancedConfig, ConfigManager
...
class TestParserBenchmarks:
    ...
    def create_varvamp_data(self, num_amplicons: int = 100, primers_per_amplicon: int = 4) -> str:
        ...
        for amp_id in range(num_amplicons):
            ...
            for i in range(primers_per_amplicon // 2):
                lines.append("\t".join([
                    amp_name, str(amp_length), f"FW_{amp_id}_{i}", "1",
                    str(100 + i * 20), str(120 + i * 20), "ATCGATCGATCGATCGATCG", "20",
                    "0.5", "60.0", "0.52", "58.5", "85.2"
                ]))
            
            # Reverse primers
            for i in range(primers_per_amplicon // 2):
                lines.append("\t".join([
                    amp_name, str(amp_length), f"RW_{amp_id}_{i}", "1",
                    str(380 - i * 20), str(400 - i * 20), "CGATCGATCGATCGATCGAT", "20",
                    "0.5", "60.0", "0.52", "58.5", "85.2"
                ]))
    ...
    def create_artic_data(self, num_amplicons: int = 100) -> str:
        ...
        for amp_id in range(num_amplicons):
            # Forward primer
            lines.append("\t".join([
                "ref", str(100 + amp_id * 400), str(120 + amp_id * 400), f"nCoV-2019_400_{amp_id}_LEFT_1", "1", "+", "ATCGATCGATCGATCGATCG"
            ]))
            
            # Reverse primer  
            lines.append("\t".join([
                "ref", str(480 + amp_id * 400), str(500 + amp_id * 400), f"nCoV-2019_400_{amp_id}_RIGHT_1", "1", "-", "CGATCGATCGATCGATCGAT"
            ]))
...
class TestRegistryBenchmarks:
    ...
    def create_varvamp_data(self, num_amplicons: int = 10) -> str:
        ...
        for i in range(num_amplicons):
            lines.append(f"amp_{i}\t300\tFW_{i}\t1\t100\t120\tATCGATCGATCGATCGATCG\t20\t0.5\t60.0\t0.52\t58.5\t85.2")
            lines.append(f"amp_{i}\t300\tRW_{i}\t1\t380\t400\tCGATCGATCGATCGATCGAT\t20\t0.5\t60.0\t0.52\t58.5\t85.2")
