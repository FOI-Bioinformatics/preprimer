...
class TestPrimerConverterErrorHandling:
    ...
    def test_convert_validation_error_reraise(self, mock_registry):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bed') as input_file:
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                with patch.object(self.converter, '_validate_amplicons') as mock_validate:
    ...
    def test_convert_unexpected_validation_error_wrapped(self, mock_registry):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bed') as input_file:
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                with patch.object(self.converter, '_validate_amplicons') as mock_validate:
                    mock_validate.side_effect = RuntimeError("Unexpected validation issue")
                    ...
                    assert "Check that the input data is complete and valid" in str(error.suggestions[0])
    ...
    def test_convert_missing_reference_file_resolution(self, mock_registry):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bed') as input_file:
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                with tempfile.NamedTemporaryFile(suffix='.fasta', delete=False) as ref_file:
                ...
                try:
                    ...
                    with patch('preprimer.core.converter.writer_registry') as mock_writer_reg:
                        ...
                        mock_parser.get_reference_file.assert_called_once_with(Path(input_file.name))
    ...
    def test_convert_file_exists_no_force_error(self, mock_writer_reg, mock_registry):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bed') as input_file:
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                existing_output = Path(output_dir) / "artic" / "primers" / "V1" / "primers.scheme.bed"
    ...
    @patch('preprimer.core.converter.parser_registry')
    @patch('preprimer.core.converter.writer_registry')
    def test_convert_file_exists_force_override_config(self, mock_writer_reg, mock_registry):
    ...
    def test_validate_amplicons_invalid_sequence_validation_disabled(self):
        ...
        try:
            ...
            pytest.fail("Should not raise ValidationError when sequence validation is disabled")
        ...
        with tempfile.NamedTemporaryFile(suffix='.bed') as input_file:
            with tempfile.TemporaryDirectory() as output_dir:
                with patch('preprimer.core.converter.parser_registry') as mock_registry:
                    mock_parser = Mock()
                    mock_parser.parse.return_value = [self.valid_amplicon]  # Use valid amplicon to avoid validation error
                    mock_parser.get_reference_file.return_value = None
                    mock_registry.get_parser.return_value = mock_parser
