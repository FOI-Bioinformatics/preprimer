...
class TestCrossParserCompatibility:
    ...
    def test_consistent_conversion_workflow(self, temp_output_dir):
        ...
        test_files = [
            ("varvamp", Path(__file__).parent / "test_data" / "ASFV_long" / "primers.tsv"),
            ("artic", Path(__file__).parent / "test_data" / "ASFV.scheme.bed"),
            ("olivar", Path(__file__).parent / "test_data" / "olivar_examples" / "olivar-design.csv")
        ]
        ...
        for format_name, file_path in test_files:
            ...
            try:
                output_files = convert_primers(
                    ...
                    output_formats=["fasta"],  # Use simplest format for compatibility test
