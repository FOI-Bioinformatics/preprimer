...
from preprimer.core.topology import (
    TopologyDetector, GenomeTopology, GenomeInfo, calculate_amplicon_length
)
...
class TestTopologyDetector:
    ...
    def test_organism_pattern_detection(self):
        ...
        genome_info = TopologyDetector.detect_topology(organism="Homo sapiens mitochondrion")
        assert genome_info.topology == GenomeTopology.CIRCULAR
        
        genome_info = TopologyDetector.detect_topology(organism="Mouse mitochondrial DNA")
        assert genome_info.topology == GenomeTopology.CIRCULAR
        
        # Chloroplast patterns
        genome_info = TopologyDetector.detect_topology(organism="Arabidopsis chloroplast")
        assert genome_info.topology == GenomeTopology.CIRCULAR
...
class TestCoordinateValidation:
    ...
    def test_strict_validation(self):
        ...
        warnings = TopologyDetector.validate_coordinates(coordinates, genome_info, strict=False)
        assert len(warnings) > 0
...
class TestAmpliconLengthCalculation:
    ...
    def test_circular_amplicon_length(self):
        ...
        length = calculate_amplicon_length(100, 200, GenomeTopology.CIRCULAR, genome_length)
        assert length == 101
        
        # Wrapping circular
        length = calculate_amplicon_length(16400, 200, GenomeTopology.CIRCULAR, genome_length)
        expected = (16569 - 16400) + 200 + 1  # 169 + 200 + 1 = 370
        assert length == expected
        
        # Edge case: start at last position
        length = calculate_amplicon_length(16568, 100, GenomeTopology.CIRCULAR, genome_length)
...
class TestTopologyAwareness:
    ...
    def test_amplicon_length_differences(self):
        ...
        circular_length = calculate_amplicon_length(start, end, GenomeTopology.CIRCULAR, 16569)
