...
from preprimer.core.enhanced_config import EnhancedConfig, AlignmentSettings, ValidationSettings
from preprimer.core.exceptions import PrePrimerError, ValidationError, ParserError, SecurityError
...
class TestPrimerDataProperties:
    ...
    def test_primer_data_creation_properties(self, name, sequence, start, strand, direction, pool):
...
class TestConfigurationProperties:
    ...
    def test_config_validation_properties(self, threads, timeout, min_length, max_length, min_gc, max_gc, pool):
...
class TestSecurityProperties:
    ...
    @given(
        path_components=st.lists(safe_path_component, min_size=1, max_size=5)
    )
    ...
    def test_input_validation_properties(self, field_value, max_length):
        ...
        try:
            sanitized = InputValidator.sanitize_string(field_value, max_length=max_length)
...
class TestErrorHandlingProperties:
    ...
    @given(
        ...
        context_values=st.lists(st.text(min_size=0, max_size=50), min_size=0, max_size=5)
    )
    def test_error_context_properties(self, error_message, context_keys, context_values):
