...
class TestVarVAMPParserContentParsing:
    ...
    def test_parse_file_with_no_fieldnames(self):
        ...
        try:
            ...
            assert "empty or missing headers" in error.user_message or "varvamp format" in error.user_message.lower()
        finally:
    ...
    def test_parse_file_with_empty_rows(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            header = ["amplicon_name", "amplicon_length", "primer_name", "pool", "start", "stop", "seq", 
                     "size", "gc_best", "temp_best", "mean_gc", "mean_temp", "score"]
            f.write("\t".join(header) + "\n")
            f.write("\t\t\t\t\t\t\t\t\t\t\t\t\n")  # Empty row
            f.write("amp1\t200\tFW_primer1\t1\t100\t120\tATCGATCG\t8\t50.0\t60.0\t45.0\t58.0\t0.9\n")  # Valid row
            f.write("\t\t\t\t\t\t\t\t\t\t\t\t\n")  # Another empty row
    ...
    def test_parse_file_with_invalid_primer_name_format(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            f.write("amp1\t200\tINVALID_primer1\t1\t100\t120\tATCGATCG\t8\t50.0\t60.0\t45.0\t58.0\t0.9\n")
    ...
    def test_parse_file_with_invalid_coordinates(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            f.write("amp1\t200\tFW_primer1\t1\t150\t120\tATCGATCG\t8\t50.0\t60.0\t45.0\t58.0\t0.9\n")
        ...
        try:
            ...
            assert "start position must be less than stop position" in error.user_message
        finally:
    ...
    def test_parse_file_with_specific_parser_error_reraise(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            f.write("amp1\t200\tFW_primer1\tinvalid_pool\t100\t120\tATCGATCG\t8\t50.0\t60.0\t45.0\t58.0\t0.9\n")
...
class TestVarVAMPParserFileErrors:
    ...
    def test_parse_file_not_found_error(self):
        ...
        parser = VarVAMPParser()
        with pytest.raises((ParserError, InvalidFormatError, FileNotFoundError)) as exc_info:
    ...
    def test_parse_permission_error(self):
        ...
        try:
            ...
            try:
                with pytest.raises((ParserError, PermissionError, InvalidFormatError)) as exc_info:
...
class TestVarVAMPParserSafeConversions:
    ...
    def test_safe_int_convert_invalid_value(self):
        ...
        with patch('preprimer.parsers.varvamp_parser.logger') as mock_logger:
            ...
            assert result is None
            mock_logger.warning.assert_called_once()
            assert "Invalid amplicon_length in row 3" in str(mock_logger.warning.call_args)
...
class TestVarVAMPParserComplexScenarios:
    ...
    def test_amlicon_typo_correction(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            header = ["amlicon_name", "amplicon_length", "primer_name", "pool", "start", "stop", "seq", 
                     "size", "gc_best", "temp_best", "mean_gc", "mean_temp", "score"]
            f.write("\t".join(header) + "\n")
            f.write("amp1\t200\tFW_primer1\t1\t100\t120\tATCGATCG\t8\t50.0\t60.0\t45.0\t58.0\t0.9\n")
    ...
    def test_complete_varvamp_parsing(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            ...
            f.write("\t".join(header) + "\n")
            # Forward primer
            f.write("amp1\t400\tFW_primer1\t1\t100\t120\tATCGATCGATCGATCG\t16\t50.0\t60.0\t45.0\t58.0\t0.9\n")
            # Reverse primer
            f.write("amp1\t400\tRW_primer1\t1\t380\t400\tCGATCGATCGATCGAT\t16\t52.0\t62.0\t48.0\t60.0\t0.85\n")
        ...
        try:
            ...
            forward_primer = next(p for p in amplicon.primers if p.direction == "forward")
            reverse_primer = next(p for p in amplicon.primers if p.direction == "reverse")
    ...
    def test_metadata_preservation(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as f:
            header = ["amplicon_name", "amplicon_length", "primer_name", "pool", "start", "stop", "seq", 
                     "size", "gc_best", "temp_best", "mean_gc", "mean_temp", "score"]
            f.write("\t".join(header) + "\n")
            f.write("amp1\t400\tFW_primer1\t2\t100\t120\tATCGATCG\t8\t50.0\t60.0\t45.5\t58.5\t0.95\n")
