...
class TestOlivarParserWithRealData:
    ...
    def olivar_bed_file(self):
        ...
        return Path(__file__).parent / "test_data" / "olivar_examples" / "olivar-design.primer.bed"
    ...
    def test_olivar_csv_parsing(self, olivar_csv_file):
        ...
        assert len(amp1.primers) == 2, "Each amplicon should have 2 primers (forward and reverse)"
    ...
    def test_olivar_primer_pools(self, olivar_csv_file):
        ...
        for i, amplicon in enumerate(amplicons):
            for primer in amplicon.primers:
                assert primer.pool == expected_pools[i], f"Amplicon {i+1} should be in pool {expected_pools[i]}"
    ...
    def test_olivar_conversion_to_artic(self, olivar_csv_file, reference_file):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(first_line) == 7  # BED format: chrom, start, end, name, pool, strand, sequence
...
def test_olivar_file_detection():
    ...
    olivar_file = Path(__file__).parent / "test_data" / "olivar_examples" / "olivar-design.csv"
    
    if olivar_file.exists():
        detected_format = parser_registry.detect_format(olivar_file)
        assert detected_format == "olivar", f"Expected 'olivar', got '{detected_format}'"
