...
from preprimer import convert_primers, PrePrimerConfig, PrimerConverter
...
class TestConvertPrimersMainAPI:
    ...
    def test_convert_primers_config_kwargs_processing(self):
        ...
        # Create test data
        test_input_file = Path(__file__).parent / "test_data" / "datasets" / "small" / "varvamp.tsv"
    ...
    def test_convert_primers_full_parameter_coverage(self):
        ...
        test_input_file = Path(__file__).parent / "test_data" / "datasets" / "small" / "varvamp.tsv"
        test_reference = Path(__file__).parent / "test_data" / "datasets" / "small" / "reference.fasta"
    ...
    def test_convert_primers_with_converter_mock(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_converter.convert.return_value = {"fasta": Path(temp_dir) / "test.fasta"}
    ...
    def test_convert_primers_multiple_formats(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for format_name, file_path in result.items():
                assert file_path.exists(), f"Output file for {format_name} doesn't exist"
