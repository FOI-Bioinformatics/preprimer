...
from preprimer.core.exceptions import (
    PrePrimerError, ParserError, ValidationError, AlignmentError, OutputError,
    ConfigError, SecurityError, FileNotFoundError, InvalidFormatError,
    CorruptedDataError, InsufficientDataError, ErrorContext, handle_common_exceptions
)
...
class TestAlignmentErrorMissedLines:
    ...
    def test_alignment_error_default_suggestions_with_tool(self):
        ...
        assert any("properly installed" in suggestion.lower() for suggestion in suggestions)
        assert any("reference sequences" in suggestion.lower() for suggestion in suggestions)
        assert any("primer sequences" in suggestion.lower() for suggestion in suggestions)
...
class TestSecurityErrorMissedLines:
    ...
    def test_security_error_default_user_message(self):
        ...
        expected_message = "Security violation detected. The operation was blocked for safety."
        assert error.user_message == expected_message
    ...
    def test_security_error_default_suggestions(self):
        ...
        assert any("'..' or suspicious characters" in suggestion for suggestion in suggestions)
...
class TestFileNotFoundErrorMissedLines:
    ...
    def test_file_not_found_error_default_suggestions(self):
        ...
        assert any("file exists and is readable" in suggestion for suggestion in suggestions)
...
class TestCorruptedDataErrorMissedLines:
    ...
    def test_corrupted_data_error_with_details(self):
        ...
        error = CorruptedDataError("/path/to/file.txt", details="Invalid line format at line 5")
        
        # Should include details in technical message
        assert "Invalid line format at line 5" in error.technical_message
        assert "Corrupted data in /path/to/file.txt: Invalid line format at line 5" == error.technical_message
    ...
    def test_corrupted_data_error_default_suggestions(self):
        ...
        suggestions = error.suggestions
        assert len(suggestions) == 3
        assert any("re-downloading or re-creating" in suggestion for suggestion in suggestions)
...
class TestInsufficientDataErrorMissedLines:
    ...
    def test_insufficient_data_error_default_suggestions(self):
        ...
        # Test without suggestions - should generate defaults
        error = InsufficientDataError("Not enough primers found", required_count=5, actual_count=2)
        ...
        assert any("complete and not truncated" in suggestion for suggestion in suggestions)
...
class TestExceptionEdgeCaseScenarios:
    ...
    def test_alignment_error_with_custom_user_message(self):
        ...
        error = AlignmentError("Technical error", tool="minimap2", user_message=custom_message)
    ...
    def test_alignment_error_with_custom_suggestions(self):
        ...
        error = AlignmentError("Technical error", tool="minimap2", suggestions=custom_suggestions)
    ...
    def test_output_error_without_output_path(self):
        ...
        expected_message = "Failed to write output. Please check file permissions and disk space."
        assert error.user_message == expected_message
    ...
    def test_config_error_without_config_file(self):
        ...
        expected_message = "Configuration error. Please check your configuration settings."
        assert error.user_message == expected_message
    ...
    def test_complex_scenario_with_all_defaults(self):
        ...
        try:
            ...
            raise AlignmentError(
                "Complex alignment failure", 
                tool="complex_aligner"
            )
