...
class TestCircularGenome:
    ...
    def test_circular_coordinate_wrapping_varvamp(self, mitochondrial_dataset_path):
        ...
        for amplicon in wrapping_amplicons:
            ...
            for primer in primers:
                assert len(primer.sequence) > 15, f"Primer sequence too short: {primer.sequence}"
                assert all(base in 'ATCG' for base in primer.sequence.upper()), f"Invalid bases in primer: {primer.sequence}"
    ...
    def test_circular_coordinate_wrapping_olivar(self, mitochondrial_dataset_path):
        ...
        for amplicon in amplicons:
            primers = amplicon.primers
            assert len(primers) >= 2, f"Amplicon {amplicon.amplicon_id} should have at least 2 primers"
            
            # Check coordinate ranges are reasonable for mitochondrial genome
            for primer in primers:
                assert 0 <= primer.start < 16569, f"Primer start coordinate out of range: {primer.start}"
                assert 0 <= primer.stop < 16569, f"Primer end coordinate out of range: {primer.stop}"
    
    def test_circular_conversion_consistency(self, mitochondrial_dataset_path, temp_workspace):
        ...
        assert original_seqs == converted_seqs, "Primer sequences not preserved in conversion"
    ...
    def test_circular_amplicon_validation(self, mitochondrial_dataset_path):
        ...
        for amplicon in amplicons:
            ...
            for primer in primers:
                assert len(primer.sequence) >= 18, f"Primer too short: {len(primer.sequence)}"
                assert len(primer.sequence) <= 35, f"Primer too long: {len(primer.sequence)}"
                
                # Check for valid DNA bases
                valid_bases = set('ATCGRYSWKMBDHVN')  # Including ambiguous bases
                primer_bases = set(primer.sequence.upper())
                assert primer_bases.issubset(valid_bases), f"Invalid bases in primer: {primer.sequence}"
