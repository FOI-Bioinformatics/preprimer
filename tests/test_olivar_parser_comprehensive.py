...
class TestOlivarParserContentParsing:
    ...
    def test_parse_file_with_no_fieldnames(self):
        ...
        try:
            ...
            assert "olivar format" in error.user_message or "empty" in error.user_message
        finally:
    ...
    def test_parse_file_with_unexpected_row_error(self):
        ...
        try:
            ...
            with patch.object(parser, '_create_primer_data') as mock_create:
                mock_create.side_effect = RuntimeError("Unexpected error in primer creation")
...
class TestOlivarParserAmpliconCreation:
    ...
    def test_amplicon_creation_branch_coverage(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.writer(f)
            writer.writerow(["amplicon_id", "fP", "rP", "pool", "chrom", "start", "end"])
            # Add multiple rows for same amplicon to test both branches
            writer.writerow(["amp1", "ATCGATCG", "CGATCGAT", "1", "chr1", "100", "200"]) 
            writer.writerow(["amp1", "TTTTAAAA", "AAAATTTT", "2", "chr1", "300", "400"])  # Same amplicon_id
...
class TestOlivarParserComplexScenarios:
    ...
    def test_metadata_preservation(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.writer(f)
            writer.writerow(["amplicon_id", "fP", "rP", "pool", "chrom", "start", "end", "custom_field"])
            writer.writerow(["amp1", "ATCG", "CGAT", "1", "chr1", "100", "200", "custom_value"])
...
class TestOlivarParserValidationMethods:
    ...
    def test_filename_based_detection(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='-olivar-design.csv', delete=False) as f:
