...
from preprimer.core.enhanced_config import (
    EnhancedConfig, AlignmentSettings, ValidationSettings, OutputSettings,
    ParserSettings, PluginSettings, SecuritySettings, LoggingSettings,
    ConfigManager, ConfigWatcher, get_config, update_config, update_config_partial
)
...
class TestFileConfiguration:
    ...
    def test_save_configuration(self):
        ...
        config = EnhancedConfig(
            alignment=AlignmentSettings(aligner="exonerate", threads=8),
            debug_mode=True
        )
    ...
    def test_configuration_validation_error_handling(self):
        ...
        with pytest.raises(ValueError):
            EnhancedConfig(
                ...
                validation=ValidationSettings(min_length=50, max_length=30)  # Invalid range
            )
