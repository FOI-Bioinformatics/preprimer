...
class ParserTestBase:
    ...
    def assert_parser_consistency(self, parser, file_path: Path, expected_amplicons: int):
    ...
    def assert_conversion_consistency(self, input_file: Path, expected_amplicons: int, 
                                    expected_primers: int, prefix: str = "TEST"):
    ...
    def _validate_artic_output(self, file_path: Path, expected_primers: int):
        ...
        for line in lines:
            parts = line.split('\t')
            assert len(parts) == 7  # BED format: chrom, start, end, name, pool, strand, sequence
...
class TestVarVAMPParser(ParserTestBase):
    ...
    def reference_file(self):
        ...
        return Path(__file__).parent / "test_data" / "ASFV_long" / "ambiguous_consensus.fasta"
    ...
    def test_varvamp_parsing_consistency(self, test_file):
        ...
        parser = VarVAMPParser()
        amplicons = self.assert_parser_consistency(parser, test_file, expected_amplicons=80)
    ...
    def test_varvamp_conversion_workflow(self, test_file):
        ...
        self.assert_conversion_consistency(test_file, expected_amplicons=80, expected_primers=160, prefix="ASFV")
...
class TestARTICParser(ParserTestBase):
    ...
    def test_artic_parsing_consistency(self, test_file):
        ...
        amplicons = self.assert_parser_consistency(parser, test_file, expected_amplicons)
    ...
    def test_artic_conversion_workflow(self, test_file):
        ...
        self.assert_conversion_consistency(test_file, expected_amplicons, expected_primers, prefix="ASFV")
...
class TestOlivarParser(ParserTestBase):
    ...
    def test_file(self):
        ...
        return Path(__file__).parent / "test_data" / "olivar_examples" / "olivar-design.csv"
    
    @pytest.fixture
    def reference_file(self):
        """Olivar reference file fixture."""
        return Path(__file__).parent / "test_data" / "olivar_examples" / "EPI_ISL_402124_ref.fasta"
    ...
    def test_olivar_parsing_consistency(self, test_file):
        ...
        parser = OlivarParser()
        amplicons = self.assert_parser_consistency(parser, test_file, expected_amplicons=5)
    ...
    def test_olivar_conversion_workflow(self, test_file):
        ...
        self.assert_conversion_consistency(test_file, expected_amplicons=5, expected_primers=10, prefix="COVID19")
...
class TestCrossParserCompatibility:
    ...
    def test_format_detection_consistency(self, all_test_files):
        ...
        for format_name, file_path in all_test_files.items():
            ...
            assert detected == format_name, f"Expected {format_name}, got {detected} for {file_path}"
...
def run_all_tests():
    ...
    if varvamp_file.exists():
        ...
        print(f"   Parsed {len(amplicons)} amplicons with {sum(len(a.primers) for a in amplicons)} primers")
    ...
    if artic_file.exists():
        ...
        amplicons = test_base.assert_parser_consistency(parser, artic_file, expected_amplicons)
        test_base.assert_conversion_consistency(artic_file, expected_amplicons, expected_primers, "ASFV")
        print(f"   Parsed {len(amplicons)} amplicons with {sum(len(a.primers) for a in amplicons)} primers")
