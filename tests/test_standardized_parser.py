...
from preprimer.core.exceptions import ParserError, SecurityError, InvalidFormatError
...
class TestStandardizedParserInterface:
    ...
    def test_input_validation(self):
        ...
        try:
            with patch.object(parser.input_validator, 'validate_amplicon_name', 
            ...
            with patch.object(parser.path_validator, 'sanitize_path', 
    ...
    def test_validate_numeric_field(self):
        ...
        result = parser._validate_numeric_field("50", "test_field", 1, int, min_value=0, max_value=100)
        assert result == 50
    ...
    def test_validate_primer_data(self):
        ...
        with pytest.raises(ParserError, match="Start position must be less than stop position"):
            parser._validate_primer_data("TEST_F", "ATCG", 5, 1, 1)
        
        # Test invalid sequence (sequence with invalid characters)
        with patch.object(parser.input_validator, 'validate_primer_sequence', 
    ...
    def test_finalize_amplicons(self):
        ...
        # Create test primers
        primer1 = PrimerData(
            name="TEST_F", sequence="ATCG", start=1, stop=5,
            strand="+", direction="forward", pool=1,
            amplicon_id="test_amp", reference_id="test_ref"
        )
        primer2 = PrimerData(
            name="TEST_R", sequence="CGAT", start=100, stop=104,
            strand="-", direction="reverse", pool=1,
            amplicon_id="test_amp", reference_id="test_ref"
        )
    ...
    def test_error_handling_chain(self):
        ...
        with patch.object(parser, '_parse_file_content', side_effect=ValueError("Mock error")):
            with tempfile.NamedTemporaryFile(mode='w', suffix='.mock', delete=False) as f:
                f.write("content")
                temp_path = f.name
    ...
    def test_security_error_passthrough(self):
        ...
        with patch.object(parser.input_validator, 'sanitize_string', 
