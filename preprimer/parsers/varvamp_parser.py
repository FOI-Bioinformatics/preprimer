...
class VarVAMPParser(StandardizedParser):
                ...
                with open(file_path, "r", encoding='utf-8') as f:
                    ...
                    for row_num, row in enumerate(reader, start=2):  # Start at 2 for header
                        try:
                            ...
                            amplicon_name = self._sanitize_string_field(row["amplicon_name"], "amplicon_name", row_num, 100)
                            primer_name = self._sanitize_string_field(row["primer_name"], "primer_name", row_num, 100)
                            sequence = self._sanitize_string_field(row["seq"], "sequence", row_num, 1000)
                            ...
                            if primer_name.startswith("FW"):
                                ...
                                ).add_suggestion("Check that primer names follow VarVAMP conventions")

                            # Validate numeric fields using standardized methods
                            start_pos = self._validate_numeric_field(row["start"], "start", row_num, int, min_value=0)
                            stop_pos = self._validate_numeric_field(row["stop"], "stop", row_num, int, min_value=0)
                            pool_num = self._validate_numeric_field(row.get("pool", "1"), "pool", row_num, int, min_value=0)
                            ...
                            self._validate_primer_data(primer_name, sequence, start_pos, stop_pos, row_num)

                            # Safely convert optional float fields
                            gc_content = self._safe_float_convert(row.get("gc_best"), row_num, "gc_best")
                            tm = self._safe_float_convert(row.get("temp_best"), row_num, "temp_best")
                            score = self._safe_float_convert(row.get("score"), row_num, "score")
                            mean_gc = self._safe_float_convert(row.get("mean_gc"), row_num, "mean_gc")
                            mean_temp = self._safe_float_convert(row.get("mean_temp"), row_num, "mean_temp")
                            amplicon_length = self._safe_int_convert(row.get("amplicon_length"), row_num, "amplicon_length")
                    ...
                    if processed_rows == 0:
                        ...
                        ).add_suggestion("Verify that the file contains properly formatted VarVAMP data")
    ...
    def _safe_float_convert(self, value: Optional[str], row_num: int, field_name: str) -> Optional[float]:
    ...
    def _safe_int_convert(self, value: Optional[str], row_num: int, field_name: str) -> Optional[int]:
