...
class OlivarParser(StandardizedParser):
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                required_matches = sum(
                    1 for col in required_columns if col.lower() in header_lower
                )

                # Must have all 4 required columns for official Olivar format
                if required_matches >= 4:
                    logger.debug(
                        f"Valid Olivar format detected: {required_matches}/4 required columns found"
                    )
                    return True
                ...
                has_forward = any(
                    col in header_lower
                    for col in ["fp", "forward_primer", "fwd_primer"]
                )
                has_reverse = any(
                    col in header_lower
                    for col in ["rp", "reverse_primer", "rev_primer"]
                )
                has_amplicon = any(
                    col in header_lower
                    for col in ["amplicon_id", "amplicon", "region_id"]
                )
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    if not reader.fieldnames:
                        raise InvalidFormatError(
                            ...
                            "Check that the file contains Olivar header columns"
                        )
                    ...
                    for row_num, row in enumerate(
                        ...
                        try:
                            ...
                            amplicon_id = self._sanitize_string_field(
                                row["amplicon_id"], "amplicon_id", row_num, 100
                            )
                            fwd_seq = self._sanitize_string_field(
                                row["fP"], "forward_primer", row_num, 100
                            )
                            rev_seq = self._sanitize_string_field(
                                row["rP"], "reverse_primer", row_num, 100
                            )
                            ...
                            pool_num = self._validate_numeric_field(
                                row["pool"], "pool", row_num, int, min_value=1
                            )
                            ...
                            if row.get("start") and row.get("end"):
                                start_pos = self._validate_numeric_field(
                                    row["start"], "start", row_num, int, min_value=1
                                )
                                end_pos = self._validate_numeric_field(
                                    row["end"], "end", row_num, int, min_value=1
                                )
                    ...
                    if processed_rows == 0:
                        raise InsufficientDataError(
                            ...
                            "Verify that the file contains properly formatted Olivar data"
                        )
    ...
    def get_reference_file(self, file_path: Union[str, Path]) -> Optional[Path]:
        ...
        try:
            ...
            base_name = validated_path.stem.replace("-design", "").replace(
                "olivar-", ""
            )
