    ...
    def validate_file(self, file_path: Union[str, Path]) -> bool:
        ...
        try:
            with open(file_path, "r") as f:
                ...
                # Check for required columns (forward primer fP, reverse primer
                # rP)
                required_matches = sum(
                    1 for col in olivar_required if col.lower() in header_lower
                )

                # Check for common columns
                common_matches = sum(
                    1 for col in olivar_common if col.lower() in header_lower
                )

                # Must have most required columns and at least one common column
                # Or if we have all 5 required columns, that's sufficient
                return (required_matches >= 5) or (
                    required_matches >= 4 and common_matches >= 1
                )
    ...
    def parse(
        ...
        try:
            with open(file_path, "r") as f:
                ...
                if not col_mapping:
                    raise ParserError(
                        "Could not identify required columns in Olivar file"
                    )
