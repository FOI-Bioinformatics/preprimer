...
def convert(args):
    ...
    if args.input_format == "varvamp":
        ...
        if args.reference is None:
            reference = (
                args.reference
                if args.reference
                else os.path.join(primer_info_dir, "ambiguous_consensus.fasta")
            )
        else:
    ...
    # Read artic primer file into dictionary
        ...
        if args.reference is None:
            reference = os.path.join(
                primer_info_dir, old_file_prefix + "reference.fasta"
            )
        else:
            reference = args.reference
    ...
    for output_format in args.output_format:
        if output_format == "artic":
            ...
            print("#######################################\n")
            primer_fasta_filepath = os.path.join(
            ...
            if FileHandler.check_folder_exists(
                os.path.dirname(primer_fasta_filepath), args.force
            ):
