...
Examples:
  ...
  preprimer convert --input primers.bed --output-dir schemes/ --output-formats fasta
    ...
    list_parser.add_argument("--parsers", action="store_true", help="List input formats")
    list_parser.add_argument("--writers", action="store_true", help="List output formats") 
...
def cmd_convert(args: argparse.Namespace, config: PrePrimerConfig) -> int:
    ...
    try:
        ...
        if args.validate_only:
            ...
            parser = parser_registry.get_parser(args.input_format) if args.input_format else None
            ...
            logger.info(f"✅ Validation successful: {len(amplicons)} amplicons, {sum(len(a.primers) for a in amplicons)} primers")
            return 0
