...
class PrePrimerError(Exception):
    ...
    def _log_error(self):
        ...
        logger.error(
            f"{self.__class__.__name__}: {self.technical_message}", extra=log_context
        )
...
class FileNotFoundError(ParserError):
    ...
    def __init__(self, file_path: str, **kwargs):
        ...
        super().__init__(
            f"Required file not found: {file_path}", file_path=file_path, **kwargs
        )
...
class InvalidFormatError(ParserError):
    ...
    def __init__(self, file_path: str, expected_format: Optional[str] = None, **kwargs):
        ...
        kwargs["context"] = context
        super().__init__(
            f"Invalid format for file {file_path}", file_path=file_path, **kwargs
        )
...
class CorruptedDataError(ParserError):
    ...
    def __init__(self, file_path: str, details: Optional[str] = None, **kwargs):
        ...
        if "user_message" not in kwargs:
            kwargs["user_message"] = (
                f"The file {file_path} appears to be corrupted or malformed."
            )
        ...
        super().__init__(technical_msg, file_path=file_path, **kwargs)
...
class ErrorContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.logger.error(
            f"Unexpected error in {self.operation_name}: {exc_val}", exc_info=True
        )
        return False
