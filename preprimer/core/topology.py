...
class TopologyDetector:
    ...
    KNOWN_REFERENCES = {
        ...
        "NC_045512.2": GenomeInfo(
            "NC_045512.2",
            GenomeTopology.LINEAR,
            29903,
            "SARS-CoV-2",
            "COVID-19 reference genome",
        ),
        "EPI_ISL_402124": GenomeInfo(
            "EPI_ISL_402124",
            GenomeTopology.LINEAR,
            29903,
            "SARS-CoV-2",
            "Early COVID-19 isolate",
        ),
        "LR722600.1": GenomeInfo(
            "LR722600.1",
            GenomeTopology.LINEAR,
            191232,
            "ASFV",
    ...
    def detect_topology(
        ...
        if reference_id and reference_id in cls.KNOWN_REFERENCES:
            ...
            logger.info(
                f"Detected {known.topology.value} topology for known reference {reference_id}"
            )
            return known
        ...
        if organism:
            ...
            for pattern, topology in cls.ORGANISM_PATTERNS.items():
                if pattern in organism_lower:
                    logger.info(
                        f"Detected {topology.value} topology from organism pattern '{pattern}' in '{organism}'"
                    )
                    return GenomeInfo(
                        reference_id or "unknown", topology, genome_length, organism
                    )
        ...
        if coordinates and genome_length:
            ...
            if topology != GenomeTopology.UNKNOWN:
                logger.warning(
                    f"Detected {topology.value} topology from coordinate analysis - please verify!"
                )
                return GenomeInfo(
                    reference_id or "unknown", topology, genome_length, organism
                )
    ...
    def _detect_from_coordinates(
        cls, coordinates: List[tuple], genome_length: int, wrap_threshold: float = 0.1
    ) -> GenomeTopology:
        """
        ...
        for start, end in coordinates:
            ...
            if start > (genome_length - boundary_distance) or end > (
                genome_length - boundary_distance
            ):
    ...
    def validate_coordinates(
        cls, coordinates: List[tuple], genome_info: GenomeInfo, strict: bool = False
    ) -> List[str]:
        """
        ...
        for i, (start, end) in enumerate(coordinates):
            ...
            if genome_length:
                if start < 0 or end < 0:
                    warnings.append(
                        f"Coordinate pair {i+1}: Negative coordinates not allowed"
                    )

                if start >= genome_length or end >= genome_length:
                    warnings.append(
                        f"Coordinate pair {i+1}: Coordinates exceed genome length {genome_length}"
                    )
            ...
            if topology == GenomeTopology.LINEAR:
                ...
                if start > end:
                    msg = (
                        f"Coordinate pair {i+1}: start ({start}) > end ({end}) "
                ...
                if (
                    genome_length
                    and start > (genome_length * 0.95)
                    and end < (genome_length * 0.05)
                ):
                    msg = (
                        f"Coordinate pair {i+1}: High start ({start}) with low end ({end}) "
                ...
                if genome_length:
                    ...
                    if amp_length > (genome_length * 0.5):
                        warnings.append(
                            f"Coordinate pair {i+1}: Amplicon length ({amp_length}) "
    ...
    @classmethod
    def load_from_metadata_file(
        cls, metadata_path: Union[str, Path]
    ) -> Optional[GenomeInfo]:
...
def calculate_amplicon_length(
    start: int, end: int, topology: GenomeTopology, genome_length: Optional[int] = None
) -> int:
    """
    ...
    if topology == GenomeTopology.LINEAR or start <= end:
        ...
        logger.warning(
            f"Cannot calculate amplicon length for topology {topology} "
