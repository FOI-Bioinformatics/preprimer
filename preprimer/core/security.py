...
class PathValidator:
    ...
    @classmethod
    def sanitize_path(cls, path: Union[str, Path], base_dir: Optional[Path] = None) -> Path:
        """
        ...
        if base_dir is None:
            ...
            dangerous_dirs = [
                ...
                '/boot/', '/dev/', '/proc/', '/sys/',
                '/var/mail/', '/var/log/', '/var/lib/', '/var/run/', '/var/spool/',
                'C:\\Windows\\System32\\', 'C:\\Program Files\\', 'C:\\Windows\\',
                '/home/root/', '/root/'
            ]
            
            # Allow legitimate temporary and user directories
            allowed_prefixes = [
                '/tmp/', '/var/tmp/', '/var/folders/',  # Common temp directories
                str(Path.home()),  # User home directory
            ]
            
            # Check if path is in an allowed temporary location
            is_allowed = any(path_str.startswith(allowed) for allowed in allowed_prefixes)
    ...
    @classmethod
    def is_safe_to_write(cls, path: Union[str, Path], base_dir: Optional[Path] = None) -> bool:
        """
...
class SecureFileOperations:
    ...
    def safe_create_directories(self, directory: Union[str, Path], mode: int = 0o755) -> Path:
        """
    ...
    def _is_system_directory(self, path: Path) -> bool:
        ...
        system_paths = {
            ...
            'c:\\users\\default', '/system', '/applications'
        }
...
def secure_subprocess_call(
    ...
    for i, arg in enumerate(command):
        ...
        if any(char in arg for char in dangerous_chars):
            raise SecurityError(f"Command argument contains dangerous characters: {arg}")
