...
class OlivarWriter(OutputWriter):
    ...
    def write(self, 
        ...
        for amplicon in amplicons:
            ...
            amplicon_id = f"{prefix}_{amplicon.amplicon_id}" if prefix else amplicon.amplicon_id
    ...
    def write_with_metadata(self, 
        ...
        chrom_name = 'ref'
        if metadata:
            chrom_name = metadata.get('chrom_name', metadata.get('reference_name', 'ref'))
        ...
        for amplicon in amplicons:
            ...
            if metadata:
                row.update({k: v for k, v in metadata.items() 
                          if k not in row})  # Don't overwrite existing fields
