            ...
            with open(output_path, 'w') as f:
                ...
                for amplicon in amplicons:
                    ...
                    if not forward_primers or not reverse_primers:
                        logger.warning(f"Skipping amplicon {amplicon.amplicon_id}: missing forward or reverse primer")
                        continue
                    ...
                    if len(forward_primers) > 1 or len(reverse_primers) > 1:
                        logger.info(f"Amplicon {amplicon.amplicon_id} has multiple primers per direction, using first of each")
            
            logger.info(f"Successfully wrote {amplicon_count} amplicons to STS format")
            
            # If reference was provided, log it for user info
            if reference_path:
                logger.info(f"STS file ready for use with me-pcr against reference: {reference_path}")
                logger.info(f"Example command: me-pcr -f {reference_path} -s {output_path}")
